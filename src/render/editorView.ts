...
export class EditorView {
  ...
  update(camera: Camera, viewW: number, viewH: number): void {
    ...
    // Anchors: the fixed connection points.
    for (const anchor of this.session.doc.anchors) {
      const pos = this.session.nodePosition(anchor.id) ?? anchor
      const r = 0.34 * Math.min(Math.max(scale, 0.5), 3)
      g.circle(pos.x, pos.y, r).fill(anchor.attachedTo ? 0xffc46b : 0xff9d5c)
      g.circle(pos.x, pos.y, r * 1.7).stroke({
    ...
    if (this.editor.hoverNode) {
      const pos = this.session.nodePosition(this.editor.hoverNode)
      if (pos) {
